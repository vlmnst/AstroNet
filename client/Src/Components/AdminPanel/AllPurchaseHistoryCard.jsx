...
import { getPurchasedProductsAllUsers, putpurchasedProducts, sendEmail} from "../../../Redux/Slice/userSlice";
...
const AllPurchaseHistoryCard = (props) => {
    ...
    const handlePushStatus = (type) => {
        ...
        let email = { //<---------------------- debe contener userMail = 
            userMail: UserEmail,// email de usuario
            message: "hello! astronet greets you, we want to inform you that your request has changed status, it is in "+ type +" We are here to help you with any questions Greetings and have a great day " 
        }
            dispatch(sendEmail(email))// despacha al reducer payload 
        const payload = {
            order: item.order,
            status: {
                username: userAdmin,
